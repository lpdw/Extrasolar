...
class WpPostsRepository extends EntityRepository
{
    ...
    {
        return $this->getEntityManager()
              ->createQuery("SELECT p.postTitle, p.postDate, p.guid, p.postContent, u.userLogin FROM AppBundle:WpPosts p JOIN AppBundle:WpUsers u WITH u.id = p.postAuthor where p.postStatus = 'publish' AND p.postContent LIKE '%".$name."%' ORDER BY p.postDate DESC")
              ->setMaxResults(5)
              ->getResult()
              ;
}
